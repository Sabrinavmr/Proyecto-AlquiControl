    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard
          ...
          sub={`${data.rented_properties} alquiladas`}
          icon={Building2}
          color="blue"
        />
        <StatCard
        ...
        <StatCard
          label="Por cobrar"
          value={formatCurrency(data.amount_to_collect)}
          sub={`${data.pending_payments + data.overdue_payments} pagos`}
          icon={TrendingUp}
          color={data.overdue_payments > 0 ? 'red' : 'amber'}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        ...
        <StatCard
          ...
          icon={AlertTriangle}
          color={data.overdue_payments > 0 ? 'red' : 'gray'}
        />
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mt-6">
        {[
          ...
          <Link
            key={to}
            to={to}
            className="flex items-center gap-3 bg-white border border-gray-100 rounded-2xl px-4 py-4 hover:border-blue-200 hover:bg-blue-50 transition-all group"
          >
